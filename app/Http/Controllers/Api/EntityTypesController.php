...
class EntityTypesController extends BaseDataConfigController
{
     ...
     * 获取验证规则
     *
     * 功能描述：定义实体类型数据的验证规则，包括创建和更新时的不同规则
     *
     * 传入参数：
     ...
     * 输出参数：
     * - array: 验证规则数组
     *   - name (string): 名称，必填，字符串，最大100字符
     *   - code (string): 编码，必填，字符串，最大50字符，唯一性验证
    ...
    protected function getValidationRules($isUpdate = false)
    {
        // 定义基础验证规则
        $rules = [
            'name' => 'required|string|max:100',           // 名称：必填，字符串，最大100字符
            'code' => 'required|string|max:50',            // 编码：必填，字符串，最大50字符
        ...
        if ($isUpdate) {
            // 更新时，排除当前记录的唯一性验证
            $id = request()->route('id');
            $rules['code'] .= '|unique:entity_types,code,' . $id;
        } else {
            // 创建时，全局唯一性验证
