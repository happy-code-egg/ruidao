 ...
 *
 * 输出参数：
 * - string: 模型类名 FollowUpType::class
 */
 ...
 *
 * 功能描述：定义跟进类型数据的验证规则，包括创建和更新时的不同规则
 *
 * 传入参数：
 ...
 * 输出参数：
 * - array: 验证规则数组
 *   - name (string): 跟进类型名称，必填，字符串，最大100字符
 *   - code (string): 跟进类型编码，必填，字符串，最大50字符，唯一性验证
...
protected function getValidationRules($isUpdate = false)
{
    // 定义基础验证规则
    $rules = [
        'name' => 'required|string|max:100',           // 跟进类型名称：必填，字符串，最大100字符
        'code' => 'required|string|max:50',            // 跟进类型编码：必填，字符串，最大50字符
...
protected function getValidationMessages()
{
    ...
    return array_merge(parent::getValidationMessages(), [
        'name.required' => '跟进类型名称不能为空',      // 名称必填验证消息
        'code.required' => '跟进类型编码不能为空',      // 编码必填验证消息
        'code.unique' => '跟进类型编码已存在',          // 编码唯一性验证消息
    ]);
}
 ...
 *
 * 功能描述：获取跟进类型列表，支持按名称和状态搜索，支持分页
 *
 * 传入参数：
 * - name (string, optional): 跟进类型名称搜索关键词
