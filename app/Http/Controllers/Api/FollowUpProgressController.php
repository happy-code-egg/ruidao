 ...
 *
 * 输出参数：
 * - string: 模型类名 FollowUpProgress::class
 */
 ...
 *
 * 功能描述：定义跟进进度数据的验证规则，包括创建和更新时的不同规则
 *
 * 传入参数：
 ...
 * 输出参数：
 * - array: 验证规则数组
 *   - name (string): 跟进进度名称，必填，字符串，最大100字符
 *   - code (string): 跟进进度编码，必填，字符串，最大50字符，唯一性验证
 *   - percentage (int): 进度百分比，必填，整数，0-100之间
...
protected function getValidationRules($isUpdate = false)
{
    // 定义基础验证规则
    $rules = [
        'name' => 'required|string|max:100',           // 跟进进度名称：必填，字符串，最大100字符
        'code' => 'required|string|max:50',            // 跟进进度编码：必填，字符串，最大50字符
        'percentage' => 'required|integer|min:0|max:100', // 进度百分比：必填，整数，0-100
...
protected function getValidationMessages()
{
    ...
    return array_merge(parent::getValidationMessages(), [
        'name.required' => '跟进进度名称不能为空',      // 名称必填验证消息
        'code.required' => '跟进进度编码不能为空',      // 编码必填验证消息
        'code.unique' => '跟进进度编码已存在',          // 编码唯一性验证消息
        'percentage.required' => '进度百分比不能为空',   // 百分比必填验证消息
        'percentage.integer' => '进度百分比必须是整数',  // 百分比整数验证消息
        'percentage.min' => '进度百分比不能小于0',      // 百分比最小值验证消息
        'percentage.max' => '进度百分比不能大于100',    // 百分比最大值验证消息
    ]);
}
 ...
 *
 * 功能描述：获取跟进进度列表，支持按名称和状态搜索，支持分页
 *
 * 传入参数：
 * - name (string, optional): 跟进进度名称搜索关键词
